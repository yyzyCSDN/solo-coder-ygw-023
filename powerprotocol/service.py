...
from .domain import ProtocolGateway

class PowerProtocolService(Engine):
    def __init__(self, database: str=":memory:") -> None:
        ...
        self.domain=ProtocolGateway(self)
    def export(self, fmt: str="json") -> str:
        return self.export_csv() if fmt.lower()=="csv" else self.operations.json_snapshot()

def demo() -> None:
    with tempfile.TemporaryDirectory(prefix="powerprotocol-") as folder:
        service=PowerProtocolService(str(Path(folder)/"demo.db"))
